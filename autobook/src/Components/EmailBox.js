...
export default function EmailBox(props) {
  ...
  function rejectCollab(collabID) {
    Axios.put(
      `http://localhost:3001/reject-collab/${collabID}/shops/${props.shopID}`
    )
      .then((res) => {
      ...
      html: `
        <select id="collab--product--value" class="swal2-input">
          ...
          ${Object.keys(inputOptions)
            .map(
              (key) => `<option value="${key}">${inputOptions[key]}</option>`
            )
            .join("")}
        </select>
  ...
  function updateCollab(collabID, selectedProductID, discountPrice) {
    ...
    if (product) {
      ...
      Axios.put(
        `http://localhost:3001/accept-collab/${collabID}/shops/${props.shopID}`,
        {
      ...
      Axios.put(`http://localhost:3001/push-shop-collab/${props.shopID}`, {
        ...
        collabs.map((collab) => (
          <div className="collab-req" key={collab._id}>
            ...
            <span className="req--date">
              {new Date(collab.createdDate).toLocaleDateString()}
            </span>
            <button
              className="accept-button"
              onClick={() => acceptCollab(collab._id)}
            >
              Accept
            </button>
            <button
              className="reject-button"
              onClick={() => rejectCollab(collab._id)}
            >
              Reject
            </button>
          </div>
