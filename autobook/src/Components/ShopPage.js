  ...
  useEffect(()=>{
    ...
    const fetchData= async ()=>{
        ...
        if(productIDs.length > 0){
            await Axios.post("http://localhost:3001/get-shop-products",{productIDs : productIDs})
             .then((res)=>{
  ...
  return (
    <div>
      ...
      <div className="shop--page--banner--holder">
        ...
        <img src={require('../Images/back--button--left.svg').default} alt="back--button" className="back--button--shop--page" onClick={()=>navigate(-1)}/>
      ...
      <div className="product--container--shop--page">
            {products.length > 0 ? productCardArray : <div>Loading...</div>}
      </div>
    </div>
  );
}
